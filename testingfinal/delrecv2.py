...
#print phone
	...
	data_item_field = {"first-name": 0, "last-name": 1, "DOB": 2, "phone-number": 3, "SSN": 4, "email": 5 }
    ...
    for i in files:
        ...
        if i[0] == "#": #or (not line [0].isdigit()) :
           ...
           # print "x:", x, type(x[0])
			...
			if x[0].split(",")[data_item_field[data_item]][0:2] != deletion_month:     #!= 'Logan':
					data.append(x[0].split(","))
					pass #???
			else:
				if x[0].split(",")[data_item_field[data_item]] != del_value:     #!= 'Logan':
					data.append(x[0].split(","))
